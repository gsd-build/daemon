...
func TestHandleSessionTitleRequestSendsGeneratedTitle(t *testing.T) {
	root := t.TempDir()
	client := relayClientStub(false)
	var got sessionTitleGenerateRequest
	d := &Daemon{
		cfg:     &config.Config{MachineID: "m1", RelayURL: "wss://localhost/ws"},
		version: "test",
		manager: &mockManager{},
		client:  client,
		generateSessionTitle: func(ctx context.Context, req sessionTitleGenerateRequest) (string, error) {
			got = req
			return "AI Chat Titles", nil
		},
	}
	d.channelRoots.Store("chan-title", root)

	err := d.handleMessage(&protocol.Envelope{Payload: &protocol.SessionTitleRequest{
		Type:         protocol.MsgTypeSessionTitleRequest,
		SessionID:    "sess-title",
		ChannelID:    "chan-title",
		RequestID:    "title-1",
	...
	env, err := drainQueuedMessage(t, client)
	if err != nil {
		t.Fatalf("drain title result: %v", err)
	}
	result, ok := env.Payload.(*protocol.SessionTitleResult)
	if !ok {
		t.Fatalf("payload = %T, want SessionTitleResult", env.Payload)
	}
	if !result.OK || result.Title != "AI Chat Titles" || result.Source != "pi" {
		t.Fatalf("result = %#v", result)
	}
	if result.ExpectedName != "Can We Add" {
		t.Fatalf("expected name = %q", result.ExpectedName)
	}
	if got.FirstMessage != "Can we add AI-generated chat titles?" {
		t.Fatalf("first message = %q", got.FirstMessage)
	}
	if got.CWD != root {
...
func TestHandleSessionTitleRequestSendsGeneratorError(t *testing.T) {
	client := relayClientStub(false)
	d := &Daemon{
		cfg:     &config.Config{MachineID: "m1", RelayURL: "wss://localhost/ws"},
		version: "test",
		...
		client:  client,
		generateSessionTitle: func(ctx context.Context, req sessionTitleGenerateRequest) (string, error) {
			return "", fmt.Errorf("title generation timed out")
		},
	}
	...
	if result.OK || result.Title != "" || result.Error != "title generation timed out" {
		t.Fatalf("result = %#v", result)
	}
}
