...
const DISABLED_BROWSER_METHOD_CATEGORIES = new Set<BrowserToolCategory>(["credential_auth"]);

export const BROWSER_TOOL_METHODS = Object.entries(BROWSER_METHOD_CATEGORY)
  .filter(([, category]) => !DISABLED_BROWSER_METHOD_CATEGORIES.has(category))
  .map(([method]) => method) as BrowserToolMethod[];
